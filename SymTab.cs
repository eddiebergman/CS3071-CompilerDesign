...
// close current scope
   public void CloseScope() {
      ...
      Console.WriteLine("Level : {0} - {1}"
        ,curLevel, curLevel == 0? "global": "local");
      Console.WriteLine(";=======");
      for(Obj cur = topScope.locals; cur != null; cur = cur.next){
          if(cur.kind == var){
              Console.WriteLine("[{0}]\t{1}\t\t: {2}", cur.adr, cur.name, ObjtoString(cur));
          }else{
              Console.WriteLine("{0}\t{1}", ObjtoString(cur), cur.name);
          }
      }
      Console.WriteLine(";=======\n");
   ...
   private string ObjtoString(Obj obj){
       if(obj.kind == var){
           ...
           return obj.kind == proc? "proc" : "scope";
       }
   }
